...
class ueDetailView(APIView):
    ...
    def get(self, request):
        ...
        if id != None:
            ue = UE.objects.filter(id=str(id))
            if ue.exists():
...
class supprimerUeView(APIView):
    ...
    def delete(self, request):
        id = request.GET.get(self.lookup_url_kwarg)
        if id != None:
            ue_result = UE.objects.filter(id=str(id))
            if ue_result.exists():
                ue = ue_result[0]
                ue.delete()
